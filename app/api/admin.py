...
@router.post(
    "/talleres",
    response_model=TallerCreadoResponse,
    status_code=status.HTTP_201_CREATED,
    summary="Registrar nuevo taller (crea su tenant + subdominio)",
)
def crear_taller(
    ...
    """
    Cada taller es un TENANT independiente (multi-tenant del enunciado).
    Al registrarlo se crea automaticamente:
      - el tenant con slug derivado del nombre (= su subdominio)
      - el taller vinculado a ese tenant
      - su suscripcion al plan free (si existe)
    Devuelve el `slug` para que el front muestre la URL del taller.
    """
    from app.core.slug import unique_tenant_slug
    ...
    tok = current_tenant.set(0)  # bypass del filtro: estamos creando un tenant nuevo
    try:
        # 1) Tenant con slug unico derivado del nombre del taller
        slug = unique_tenant_slug(db, payload.nombre)
        tenant = Tenant(
            slug=slug,
        ...
        # 2) Suscripcion al plan free (best-effort, igual que en /signup)
        plan_free = db.query(Plan).filter(Plan.codigo == "free").first()
        if plan_free:
            db.add(Suscripcion(id_tenant=tenant.id_tenant, id_plan=plan_free.id_plan, estado="activa"))

        # 3) Taller vinculado al tenant
        taller = Taller(
    ...
    # Exponer el slug en la respuesta (atributo transitorio leido por Pydantic).
    # Usamos la variable local `slug` para no re-cargar el tenant tras el commit.
    taller.slug = slug  # type: ignore[attr-defined]
    return taller
